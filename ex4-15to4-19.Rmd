...
# which ignore NA comparison, pick will not ignore but create more NA in your choice. 
# pick => sum(na.rm=T)
# regex 
View(subsample)
# 2
...
tb_months[whichIsMax] |> names()

## Or
johnDoe$data$發現月 |> table() -> tb_months
whichIsMax <- which.max(tb_months)
tb_months[whichIsMax] |> names()
```
...
factor(johnDoe$data$發現季節, levels=c("spring", "summer", "fall", "winter")) -> johnDoe$data$發現季節
table(johnDoe$data$發現季節)
...
```{r, echo=F}
pick2000 <- wdi$data$year == 2000
data2000 <- wdi$data[pick2000, ]
...
```{r, echo=F}
pick_not_country <- 
  data2000$iso2c %in% iso2c_nonCountry
pick_country <- !(data2000$iso2c %in% iso2c_nonCountry)
pick_country <- !pick_not_country
subsample2000 <- data2000[pick_country, ]
...
```{r}
subsample2000$EG.USE.PCAP.KG.OE
```
